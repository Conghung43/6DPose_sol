...
public class LabelControl : MonoBehaviour
{
    ...
    private void OnPlayAnimation(object sender, EventManager.OnStageIndexEventArgs e)
    {
        ...
        if (StationStageIndex.FunctionIndex == "Sample" && StationStageIndex.ModelTargetFound && gameObject.name.Contains(StationStageIndex.stageIndex.ToString()))
        {
    ...
    {
        // Check conditions to show or hide the game object
        if (functionIndex == "Sample" && StationStageIndex.ModelTargetFound && gameObject.name.Contains(StationStageIndex.stageIndex.ToString()))
        {
    ...
    {
        // Check conditions to show or hide the game object
        if (imageTargetFound && StationStageIndex.FunctionIndex == "Sample" && StationStageIndex.ModelTargetFound && gameObject.name.Contains(StationStageIndex.stageIndex.ToString()))
        {
            gameObject.SetActive(true);
        }
        else
